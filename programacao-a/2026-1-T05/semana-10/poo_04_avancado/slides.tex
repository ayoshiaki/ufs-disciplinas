...
\begin{frame}[fragile,shrink]{O Problema: Boilerplate Repetitivo}
...
\begin{frame}[fragile,shrink]{@classmethod como Construtor Alternativo}
\begin{lstlisting}
@dataclass
class Lanche:
    nome: str
...
\begin{frame}[fragile,shrink]{Por Que Não Uma Função Comum?}
\begin{columns}[T]
\column{0.48\textwidth}
\textbf{Função livre:}
\begin{lstlisting}
...
\begin{frame}[shrink]{Guia de Decisão}
\begin{center}
\begin{tikzpicture}[
  node distance=1.2cm,
...
\begin{frame}[fragile,shrink]{LSP --- O Problema Clássico}
\begin{lstlisting}
class Retangulo:
...
\begin{frame}[fragile,shrink]{Strategy --- Composição}
\begin{lstlisting}
...
\begin{frame}[fragile,shrink]{Strategy Pythônico: Funções como Estratégias}
\begin{lstlisting}
...
\begin{frame}[fragile,shrink]{Factory Method --- Uso e Extensão}
\begin{lstlisting}
...
\begin{frame}[fragile,shrink]{O Código ``Legado'' (Procedural)}
\begin{lstlisting}
# Listas paralelas
...
\begin{frame}[fragile,shrink]{Versão OOP Refatorada --- Hierarquia}
\begin{lstlisting}
from abc import ABC, abstractmethod
from dataclasses import dataclass, field
...
\begin{frame}[fragile,shrink]{Versão OOP Refatorada --- Pedido + Strategy}
\begin{lstlisting}
@dataclass
class Pedido:
    cliente: str
