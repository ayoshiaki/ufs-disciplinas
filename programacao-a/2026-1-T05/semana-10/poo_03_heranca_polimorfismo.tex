...
\begin{frame}{O Relacionamento ``É Um''}
  ...
  \begin{center}
    \begin{tabular}{lcc}
      ...
      Pedido \emph{tem} Lanches   & ✓ & Composição \\
      \bottomrule
    \end{tabular}
  \end{center}
...
\begin{frame}[fragile]{Duck Typing na Prática}
  \begin{lstlisting}
...
\begin{frame}[fragile]{Composição no Código}
  \begin{lstlisting}
...
\begin{frame}[fragile]{O Problema: Métodos ``Esquecidos''}
  \begin{lstlisting}
class Funcionario:
...
\begin{frame}[fragile]{Solução: \texttt{ABC} e \texttt{@abstractmethod}}
  \begin{lstlisting}
...
\begin{frame}[fragile]{Subclasses Concretas}
  \begin{lstlisting}
class Atendente(Funcionario):
    def calcular_salario(self):
...
\begin{frame}[fragile]{Loop Polimórfico para a Folha}
  \begin{lstlisting}
equipe = [
