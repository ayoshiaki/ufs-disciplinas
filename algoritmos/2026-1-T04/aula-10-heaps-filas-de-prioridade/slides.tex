...
\begin{frame}{Heap Sort: complexidade}
  \begin{columns}[T]
    ...
    \begin{tikzpicture}[scale=0.9]
      \draw[->] (0,0)--(4.2,0) node[right]{\footnotesize $n$};
      \draw[->] (0,0)--(0,3.2) node[above]{\footnotesize tempo};
      \draw[domain=0.2:4, smooth, very thick, UFSRed]
        plot (\x,{0.18*\x*\x}) node[right]{\scriptsize $O(n^2)$};
      \draw[domain=0.2:4, smooth, very thick, UFSBlue]
        plot (\x,{0.5*\x*ln(\x+1)}) node[right]{\scriptsize $O(n\log n)$};
    \end{tikzpicture}
    \end{column}
    \begin{column}{0.5\textwidth}
      \begin{block}{Análise}
