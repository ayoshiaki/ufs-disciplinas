...
\begin{frame}{Por que análise amortizada?}%% 49
  ...
  \begin{exampleblock}{Análise amortizada pelo método do potencial}
    ...
    Somando sobre $m$ operações: custo total $\leq \sum \text{amortizado} + \Phi_0$.
  \end{exampleblock}
\end{frame}
...
\begin{frame}[shrink=10]{Por que surge $\alpha(n)$? (intuição)}%% 55
  \begin{columns}[T]
    \begin{column}{0.42\textwidth}
      \begin{block}{Dois tipos de nó no caminho de Find-Set}
        \textbf{Nó de bloco} (amarelo): seu pai está no \emph{mesmo bloco de ranks}.\\[4pt]
        \textbf{Nó chave} (vermelho): seu pai está em \emph{bloco superior}.\\[6pt]
        A contagem mostra:
        \begin{itemize}
          \item Nós chave por Find: $O(\alpha(n))$
          \item Custo de nós de bloco: amortizado $O(1)$
        \end{itemize}
      \end{block}
    \end{column}
    \begin{column}{0.55\textwidth}
      \begin{tikzpicture}[
        key/.style={circle,draw=UFSRed,fill=UFSRed!20,very thick,
          minimum size=0.6cm,font=\scriptsize\bfseries},
        blk/.style={circle,draw=UFSYellow!80!black,fill=UFSYellow!30,very thick,
          minimum size=0.6cm,font=\scriptsize\bfseries},
        rt/.style={circle,draw=UFSBlue,fill=UFSBlue!15,minimum size=0.6cm,
          font=\scriptsize\bfseries},
        ar/.style={-{Stealth[scale=0.7]},thick,UFSGray},
        hl/.style={-{Stealth[scale=0.9]},very thick,UFSBlue!60}
      ]
        %% Path from leaf to root: leaf-blk-blk-key-blk-key-root
        \node[rt]  (r)  at (0,4.8) {raiz};
        \node[key] (k1) at (0,3.8) {$k_1$};
        \node[blk] (b1) at (0,2.8) {$b_1$};
        \node[blk] (b2) at (0,1.8) {$b_2$};
        \node[key] (k2) at (0,0.8) {$k_2$};
        \node[blk] (b3) at (0,-0.2){$b_3$};
        \draw[ar] (b3)--(k2); \draw[ar] (k2)--(b2);
        \draw[ar] (b2)--(b1); \draw[ar] (b1)--(k1);
        \draw[ar] (k1)--(r);
        %% Block labels
        \node[font=\tiny,UFSGray,right=0.5cm of r]  {$B_3$};
        \node[font=\tiny,UFSRed,right=0.5cm of k1]  {chave: muda bloco};
        \node[font=\tiny,UFSYellow!80!black,right=0.5cm of b1] {bloco $B_2$};
        \node[font=\tiny,UFSYellow!80!black,right=0.5cm of b2] {bloco $B_2$};
        \node[font=\tiny,UFSRed,right=0.5cm of k2]  {chave: muda bloco};
        \node[font=\tiny,UFSYellow!80!black,right=0.5cm of b3] {bloco $B_1$};
        \node[font=\tiny,UFSBlue] at (0,-0.9)
          {2 nós chave $\Rightarrow$ $O(\alpha(n))$ por Find};
      \end{tikzpicture}
    \end{column}
  \end{columns}
\end{frame}

\begin{frame}{Blocos de rank (intuição)}%% 56
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \begin{block}{Definição}
...
\begin{frame}{Próximos tópicos}%% 77
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{Construindo sobre Union-Find}
        \begin{itemize}
          \item Algoritmo de Kruskal completo e prova de corretude
          \item Algoritmo de Borůvka e variantes
          \item Minimum Spanning Forest
        \end{itemize}
      \end{block}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{block}{Aprofundando a análise}
        \begin{itemize}
          \item Análise amortizada: método do potencial completo
          \item Funções de Ackermann generalizadas
          \item Lower bounds para Union-Find
        \end{itemize}
      \end{block}
    \end{column}
  \end{columns}
...
\begin{frame}{Dúvidas?}%% 78
  \begin{center}
    \vspace{0.5cm}
    {\Large Conjuntos Disjuntos: Union-Find}\\[0.5cm]
    {\large Path Compression + Union by Rank}\\[0.5cm]
    {\large $\Rightarrow$ $O(m\,\alpha(n))$}\\[1cm]
    \texttt{andreyoshiaki@dcomp.ufs.br}\\[0.3cm]
