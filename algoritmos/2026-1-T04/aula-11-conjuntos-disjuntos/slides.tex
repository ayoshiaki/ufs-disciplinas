...
\begin{frame}[shrink=12]{Custo amortizado: $\hat c = c + \Delta\Phi$}%% 56d
  \begin{columns}[T]
    \begin{column}{0.4\textwidth}
      \begin{block}{\textsc{Link}$(x,y)$: $O(\alpha(n))$}
        Custo real $O(1)$.
        \begin{itemize}
          \item $y$: rank $+1$ (se iguais) $\Rightarrow \Delta\phi(y)\leq \alpha(n)$
          \item $x$ vira não-raiz $\Rightarrow \phi(x)$ \textbf{não aumenta} ($\Delta\phi(x)\leq 0$)
          \item demais nós: inalterados
        \end{itemize}
        $\Delta\Phi \leq \alpha(n) \Rightarrow \hat c = O(\alpha(n))$.
      \end{block}
    \end{column}
    \begin{column}{0.58\textwidth}
      \begin{block}{\textsc{Find}$(x)$: caminho com $k$ nós}
        Custo real $\propto k$. \textbf{Nenhum} potencial aumenta. Classifique:
        \begin{enumerate}
          \item raiz $+$ filho da raiz ($2$): $\Delta\phi\leq 0$
          \item fronteira ($\leq \alpha(n)$, uma por valor de $\mathrm{level}$): $\Delta\phi\leq 0$
            --- custo real pago direto
          \item demais ($\geq k-\alpha(n)-2$): $\mathrm{iter}$ sobe $\Rightarrow \phi$ \textbf{cai} $\geq 1$
        \end{enumerate}
      \end{block}
      \begin{exampleblock}{Conta}
        \vspace{-0.2cm}
        \[ \Delta\Phi \;\leq\; -\,(k-\alpha(n)-2) \]
        \vspace{-0.45cm}
        \[ \hat c_{\textsc{Find}} = k + \Delta\Phi \;\leq\; \alpha(n)+2 \;=\; O(\alpha(n)) \]
      \end{exampleblock}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Limite da análise amortizada}%% 57
  \begin{block}{Custo total de $m$ operações}
    \begin{description}
      ...
      \item[\textsc{Link} (parte do \textsc{Union}):] $\leq n-1$ vezes, $O(1)$ \emph{real} cada → total $O(n)$
      \item[\textsc{Find-Set}:] saltos de bloco: $O(\alpha(n))$ por chamada;\\
        passos no mesmo bloco: amortizados ao potencial → $O(1)$
    \end{description}
    \vspace{2pt}
    \footnotesize Cada \textsc{Union} $=$ \textsc{Find-Set}$\times 2 + $\textsc{Link}, logo herda o $O(\alpha(n))$ dos \textsc{Find-Set}.
  \end{block}
  \vspace{0.3cm}
